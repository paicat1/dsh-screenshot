...
- **摆** — 一键全屏即拍；框选前整个桌面保持可操作，所有窗口像布置画面一样自由移动、缩放；**鼠标悬停任意窗口即亮起吸附边框，点一下直接截该窗口——被遮挡也能拿到完整内容**（传统控制台窗口除外，见下）——所得即所见。
...
- `Ctrl+Alt+S` — 截图：进入选取后，**点一下桌面 = 全屏**；**鼠标悬停窗口会亮起吸附边框，点一下 = 截该窗口完整内容（被遮挡也一样，传统控制台窗口除外）**；**拖拽 = 自由选区**（Esc 取消）
...
**已知特例：传统控制台窗口**（`ConsoleWindowClass`，由系统 conhost 渲染）。这类窗口包括：**cmd（命令提示符）、Windows PowerShell 5.1、独立运行的 PowerShell 7**，以及任何在传统控制台里运行的命令行程序（`python`、`node`、`git`、批处理等）——**Windows Terminal 不在其列**（它有独立的渲染引擎，机制不同）。它们**不响应 `PrintWindow`**，且**后台被覆盖时不会渲染 GDI 表面**——两条"穿透遮挡"的路径都走不通。单击这类窗口会回退为截取屏幕上该区域的可见像素：无遮挡时内容完整干净，被其他窗口遮挡时会截到遮挡内容（与 Windows 自带截图工具行为一致）。**要截干净的控制台窗口，要么先把它切到前台、移开遮挡后再单击截取，要么直接用拖拽框选截取**。
