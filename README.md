...
**已知特例：传统控制台窗口**（`ConsoleWindowClass`，如独立运行的 PowerShell 7 / cmd，非 Windows Terminal）。这类窗口是系统 conhost 渲染的，**不响应 `PrintWindow`**，且**后台被覆盖时不会渲染 GDI 表面**——两条"穿透遮挡"的路径都走不通。单击这类窗口时会回退为截取屏幕上该窗口区域的可见像素：无遮挡时内容完整干净，被其他窗口遮挡时会截到遮挡内容（与 Windows 自带截图工具行为一致）。要截干净的控制台窗口，先把它切到前台、移开遮挡即可。
